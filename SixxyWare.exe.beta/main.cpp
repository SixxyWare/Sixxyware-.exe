...
auto name = skCrypt("SixxyWare");
auto ownerid = skCrypt("R5JydaHfU0");
auto secret = skCrypt("eb0bde76e511eeceef4ae515004b84e37defe14d46f4a1fdbe522a2365a51550");
...
int main(/* credits to AGC / A GREAT CHAOS / KEN CARSON / mogus (lua env) */)
{
    ...
    std::string consoleTitle = skCrypt("SixxyWare").decrypt();
    ...
    if (std::filesystem::exists("silence.json"))
    {
        if (!CheckIfJsonKeyExists("silence.json", "username"))
        {
            ...
            std::cout << skCrypt("SixxyWare > auto logged in\n");
        }
        else
        {
